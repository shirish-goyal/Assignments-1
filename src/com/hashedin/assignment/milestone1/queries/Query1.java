...
public class Query1 extends Query {
	...
	public Query1(List<Titles> netfixTitlesList) {
		super();
		this.netfixTitlesList = netfixTitlesList;
	}

	public void execute() {
		System.out.println("Query1: List the first n records where type?");
		...
		List<Titles> reqestedDataList = netfixTitlesList.stream()
				.filter(title -> title.getType().equalsIgnoreCase(titleType)).limit(n).collect(Collectors.toList());
